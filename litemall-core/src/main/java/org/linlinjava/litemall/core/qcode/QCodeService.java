...
public class QCodeService {
    ...
    public String createFriendShareImage(Integer shareUserId) {
        try {
            ...
            byte[] imageData = wxMaService.getQrcodeService()
                    .createWxaCodeUnlimitBytes("shareUserId=" + shareUserId.toString(), null, 300, false, wxMaCodeLineColor, false);
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            LitemallStorage storageInfo = storageService.store(inputStream2, imageData.length, "image/jpeg", "邀请好友" + shareUserId + ".jpg");
