...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        orderList = orderList.stream().filter(p -> !p.getOrderSn().contains("CZ")).collect(Collectors.toList());
    ...
    public Object submit(Integer userId, String body) {
        ...
        BigDecimal checkedGoodsCommission = new BigDecimal(0);
        Integer checkedGoodsScore = 0;
        Integer checkedGoodsGiveScore = 0;
        for (LitemallGoods goods : litemallGoods) {
            ...
            if (!goods.getIsConvert()) {
                ...
                checkedGoodsCommission = checkedGoodsCommission.add(goods.getCommission().multiply(new BigDecimal(litemallCart.getNumber())));
                checkedGoodsGiveScore = checkedGoodsGiveScore + (goods.getGiveScore() * litemallCart.getNumber());
            } else {
        ...
        if (litemallScoreOld.getScore() < checkedGoodsScore) {
            return ResponseUtil.fail(GOODS_UNSHELVE, "青豆不足");
        }
        ...
        order.setGiveScore(checkedGoodsGiveScore);
        order.setCommission(checkedGoodsCommission);
        order.setOrderScore(checkedGoodsScore);
        ...
        for (LitemallCart cartGoods : checkedGoodsList) {
            ...
            orderGoods.setSpecifications(cartGoods.getSpecificationId().toString());
            orderGoods.setAddTime(LocalDateTime.now());
            orderGoodsService.add(orderGoods);
        }

        // 删除购物车里面的商品信息
        cartService.clearGoods(userId, Arrays.asList(goodIds));
    ...
    public Object prepay(Integer userId, String body, HttpServletRequest request) {
        ...
        //系统内部 购物订单id
        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        String type = JacksonUtil.parseString(body, "type");//CZ 充值支付；GW 购物支付;
        //充值金额
        BigDecimal money = Optional.ofNullable(JacksonUtil.parseString(body, "money")).map(p -> new BigDecimal(p)).orElse(BigDecimal.ZERO);
        ...
        if (type.equals("GW")) {
            ...
            order = orderService.findById(userId, orderId);
            if (order == null) {
                return ResponseUtil.badArgumentValue();
            }
            if (!order.getUserId().equals(userId)) {
            ...
            outTradeNo = "CZ" + yyyyMMddHHmmss + random;
            totalFee = money;
        }
        ...
        try {
            ...
            orderRequest.setBody("订单：" + outTradeNo);
            // 元转成分
            int fee = 0;
            fee = totalFee.multiply(new BigDecimal(100)).intValue();
        ...
        if (type.equals("CZ")) {
            LitemallOrder litemallOrderCZ = new LitemallOrder();
            litemallOrderCZ.setOrderStatus(OrderUtil.STATUS_CREATE);
            litemallOrderCZ.setOrderSn(outTradeNo);
            litemallOrderCZ.setUserId(userId);
            litemallOrderCZ.setActualPrice(money);
            litemallOrderCZ.setConsignee("");
            litemallOrderCZ.setConsignee("");
            litemallOrderCZ.setMobile("");
            litemallOrderCZ.setAddress("");
            litemallOrderCZ.setMessage("");
            litemallOrderCZ.setGoodsPrice(BigDecimal.ZERO);
            litemallOrderCZ.setOrderPrice(BigDecimal.ZERO);
            litemallOrderCZ.setFreightPrice(BigDecimal.ZERO);
            orderMapper.insertSelective(litemallOrderCZ);
        }
        return ResponseUtil.ok(result);
    }
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (order != null) {
            if (orderSn.contains("CZ")) {
                ...
                userMoneyHistory.setType(MoneyUtil.cz);
                userMoneyHistory.setTypename(MoneyUtil.getName(MoneyUtil.cz));
                ...
                moneyExample.createCriteria().andUseridEqualTo(order.getUserId());
                LitemallUserMoney userMoneyOld = userMoneyMapper.selectOneByExample(moneyExample);
                userMoneyOld.setMoney(userMoneyOld.getMoney().add(new BigDecimal(totalFee)));
                userMoneyMapper.updateByPrimaryKey(userMoneyOld);
                return WxPayNotifyResponse.success("处理成功!");
            }

            Integer giveScore = Optional.ofNullable(order.getGiveScore()).orElse(0);
            BigDecimal commission = Optional.ofNullable(order.getCommission()).orElse(BigDecimal.ZERO);
            ...
            if (userFriend != null && commission.compareTo(BigDecimal.ZERO) > 0) {
                //上级的用户id
                Integer upUserid = userFriend.getUserid();
                userFriend.setCommission(userFriend.getCommission().add(commission));
                ...
                userMoneyHistory.setAddTime(LocalDateTime.now());
                userMoneyHistory.setOrderNumber(orderSn);
                userMoneyHistory.setTypename(MoneyUtil.getName(MoneyUtil.commission));
                userMoneyHistory.setType(MoneyUtil.commission);
            ...
            scoreExample.createCriteria().andUseridEqualTo(userId);
            LitemallScore litemallScoreOld = scoreMapper.selectOneByExample(scoreExample);
            Integer orderScore = Optional.ofNullable(order.getOrderScore()).orElse(0);
            litemallScoreOld.setScore(litemallScoreOld.getScore() + giveScore - orderScore);
            scoreMapper.updateByPrimaryKeySelective(litemallScoreOld);
        }
        // 检查这个订单是否已经处理过
...
//        };
    ...
    @Transactional
    public Object entPay(Integer userId, String body, HttpServletRequest request) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        String moneyStr = JacksonUtil.parseString(body, "money");
        BigDecimal money = Optional.ofNullable(moneyStr).map(p -> new BigDecimal(p)).orElse(BigDecimal.ZERO);
        //获取小程序用户信息
        LitemallUser user = userService.findById(userId);
        String openid = user.getWeixinOpenid();
        if (openid == null) {
        ...
        Date now = new Date();
        String yyyyMMddHHmmss = DateUtils.formatDate(now, "yyyyMMddHHmmss");
        String orderNumber = "CZ" + yyyyMMddHHmmss + random;
        //转换为分
        int fee = 0;
        fee = money.multiply(new BigDecimal(100)).intValue();
        //开始提现
        EntPayRequest entPayRequest = new EntPayRequest();
        entPayRequest.setMchAppid(properties.getAppId());
        entPayRequest.setOpenid(user.getWeixinOpenid());
        entPayRequest.setCheckName("NO_CHECK");
        entPayRequest.setAmount(fee);
        entPayRequest.setDescription("我的钱包提现");
        entPayRequest.setSpbillCreateIp(user.getLastLoginIp());
        entPayRequest.setPartnerTradeNo(orderNumber);
        EntPayResult result;
        try {
            result = entPayService.entPay(entPayRequest);
        } catch (WxPayException e) {
            e.printStackTrace();
            return ResponseUtil.fail(ORDER_PAY_FAIL, "提现失败");
        }
        //支付成功之后，入库记录
        LitemallUserMoneyHistory userMoneyHistory = new LitemallUserMoneyHistory();
        userMoneyHistory.setMoney(money.negate());
        userMoneyHistory.setUserid(userId);
        userMoneyHistory.setType(4);
        userMoneyHistory.setTypename("提现");
        userMoneyHistory.setOrderNumber(orderNumber);
        userMoneyHistory.setAddTime(LocalDateTime.now());
        userMoneyHistoryMapper.insert(userMoneyHistory);

        LitemallUserMoneyExample moneyExample = new LitemallUserMoneyExample();
        moneyExample.createCriteria().andUseridEqualTo(userId);
        LitemallUserMoney userMoneyOld = userMoneyMapper.selectOneByExample(moneyExample);
        userMoneyOld.setMoney(userMoneyOld.getMoney().subtract(money));
        userMoneyMapper.updateByPrimaryKey(userMoneyOld);
        return ResponseUtil.ok(result);
    }
