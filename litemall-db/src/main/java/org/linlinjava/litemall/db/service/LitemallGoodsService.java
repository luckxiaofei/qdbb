...
public class LitemallGoodsService {
    Column[] columns = new Column[]{Column.id, Column.name, Column.brief, Column.picUrl, Column.isHot, Column.isNew, Column.isConvert, Column.giveScore, Column.counterPrice, Column.retailPrice, Column.giveScore, Column.convertScore, Column.commission};
    @Resource
    ...
    public List<LitemallGoods> queryByNew(int offset, int limit) {
        LitemallGoodsExample example = new LitemallGoodsExample();
        example.or().andIsNewEqualTo(true).andIsOnSaleEqualTo(true).andIsConvertEqualTo(false).andDeletedEqualTo(false);
        example.setOrderByClause("add_time desc");
        PageHelper.startPage(offset, limit);
     ...
     * @return
     */
    public List<LitemallGoods> queryByConvert(Integer categoryId, String keyword, int offset, int limit) {

        LitemallGoodsExample example = new LitemallGoodsExample();
        LitemallGoodsExample.Criteria criteria1 = example.or();
        LitemallGoodsExample.Criteria criteria2 = example.or();
        criteria1.andIsConvertEqualTo(true).andIsOnSaleEqualTo(true).andDeletedEqualTo(false);
        criteria2.andIsConvertEqualTo(true).andIsOnSaleEqualTo(true).andDeletedEqualTo(false);
        if (!StringUtils.isEmpty(categoryId) && categoryId > 0) {
            LitemallCategory category = categoryMapper.selectByPrimaryKey(categoryId);
            if (category.getPid() == 0) {
                ...
                List<LitemallCategory> categories = categoryMapper.selectByExample(categoryExample);
                List<Integer> categoryIdList = categories.stream().map(LitemallCategory::getId).collect(Collectors.toList());
                criteria1.andCategoryIdIn(categoryIdList);
                criteria2.andCategoryIdIn(categoryIdList);
            } else {
     ...
     * @return
     */
    public List<LitemallGoods> queryOtherGoods(int offset, int limit) {
        LitemallGoodsExample example = new LitemallGoodsExample();
        example.or().andIsOnSaleEqualTo(true).andDeletedEqualTo(false).andIsConvertEqualTo(false).andIsNewEqualTo(false);
        example.setOrderByClause("add_time desc");
        PageHelper.startPage(offset, limit);
        return goodsMapper.selectByExampleSelective(example, columns);
    }
    ...
    public List<LitemallGoods> querySelective(Integer categoryId, Integer brandId, String keywords, Boolean isHot, Boolean isNew, Integer offset, Integer limit, String sort, String order) {
        LitemallGoodsExample example = new LitemallGoodsExample();
        LitemallGoodsExample.Criteria criteria1 = example.or();
        LitemallGoodsExample.Criteria criteria2 = example.or();
        ...
        criteria1.andIsConvertEqualTo(false);
        criteria2.andIsConvertEqualTo(false);
